...
func (s ShipCommander) List(inputMsg *tgbotapi.Message) {
	...
	cursor, err := strconv.ParseUint(parts[0], 10, 64)
	if err != nil {
		s.sendMessage(inputMsg.Chat.ID,
			fmt.Sprintf("Ошибка: 'cursor' должен быть числом, получено: %s", parts[0]))
		return
	}
	...
	if err != nil {
		s.sendMessage(inputMsg.Chat.ID,
			fmt.Sprintf("Ошибка: 'limit' должен быть числом, получено: %s", parts[1]))
		return
	}
	...
	if err != nil {
		s.sendMessage(inputMsg.Chat.ID, fmt.Sprintf("Вы получили весь список кораблей"))
		return
	}
